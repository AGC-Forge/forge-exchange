  ...
  <div>
    ...
    <section class="bg-white dark:bg-neutral-950 border-b border-neutral-200/50 dark:border-neutral-800/50">
      <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
        <h1 class="font-aeonik-pro-trial text-4xl sm:text-5xl font-bold text-neutral-900 dark:text-white tracking-tight mb-3">
          {{ t('docs.title') }}
        </h1>
        ...
        <div class="flex gap-10 relative">
          ...
          >
            ...
            <div class="flex items-center justify-between mb-4 lg:hidden">
              <span class="text-sm font-semibold text-neutral-900 dark:text-white">Navigation</span>
              <UButton variant="ghost" size="sm" square @click="isSidebarOpen = false">
            ...
            <button
              ...
              :key="section.id"
              @click="activeSection = section.id; isSidebarOpen = false"
              :class="[
          ...
          <div class="flex-1 min-w-0">
            <div
              ...
              :key="key"
              :ref="(el) => { if (el) (el as any).id = key }"
            >
              <div v-if="activeSection === key" class="bg-white dark:bg-neutral-900 border border-neutral-200/60 dark:border-neutral-800/60 rounded-2xl p-8">
                <!-- Section Header -->
                <div class="flex items-center gap-3 mb-8 pb-6 border-b border-neutral-200/60 dark:border-neutral-800/60">
                  <div class="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-green-50 dark:bg-green-950/50">
                    <UIcon name="i-lucide-book-open" class="h-5 w-5 text-green-600 dark:text-green-400" />
                  </div>
                  <h1 class="text-2xl font-semibold font-aeonik-pro-trial text-neutral-900 dark:text-white">
                    {{ section.title }}
                  </h1>
                </div>
                ...
                <div class="space-y-6">
                  <template v-for="(block, bi) in section.content" :key="bi">
                    <!-- Paragraph -->
                    <p v-if="block.type === 'p'" class="text-neutral-600 dark:text-neutral-300 leading-relaxed">
                      {{ block.text }}
                    </p>

                    <!-- Heading 3 -->
                    <h3 v-else-if="block.type === 'h3'" class="text-lg font-semibold text-neutral-900 dark:text-white">
                      {{ block.text }}
                    </h3>
                    ...
                    <div
                      ...
                      :class="[
                        ...
                        block.variant === 'warning' ? 'bg-amber-50 dark:bg-amber-950/30 border-amber-200/60 dark:border-amber-800/40' : '',
                        block.variant === 'info' ? 'bg-blue-50 dark:bg-blue-950/30 border-blue-200/60 dark:border-blue-800/40' : '',
                        !block.variant || block.variant === 'default' ? 'bg-neutral-50 dark:bg-neutral-800/50 border-neutral-200/60 dark:border-neutral-800' : '',
                      ]"
                    >
                      <UIcon
                        :name="
                          block.variant === 'warning' ? 'i-lucide-alert-triangle' :
                          block.variant === 'info' ? 'i-lucide-info' :
                          'i-lucide-bookmark'
                        "
                        :class="[
                          'h-5 w-5 shrink-0 mt-0.5',
                          block.variant === 'warning' ? 'text-amber-600 dark:text-amber-400' : '',
                          block.variant === 'info' ? 'text-blue-600 dark:text-blue-400' : '',
                          !block.variant || block.variant === 'default' ? 'text-neutral-500 dark:text-neutral-400' : '',
                        ]"
                      />
                      <div>
                        <p v-if="block.title" class="text-sm font-semibold text-neutral-900 dark:text-white mb-1">
                          {{ block.title }}
                        </p>
                    ...
                    <ul v-else-if="block.type === 'list'" class="space-y-2.5">
                      ...
                      >
                        <UIcon name="i-lucide-check-circle-2" class="h-4 w-4 text-green-500 shrink-0 mt-0.5" />
                        {{ item }}
                      </li>
                    </ul>

                    <!-- Code -->
                    <div v-else-if="block.type === 'code'" class="bg-neutral-900 dark:bg-black rounded-xl overflow-hidden">
                      <div class="flex items-center justify-between px-4 py-2.5 border-b border-white/5">
                        ...
                        <span class="text-xs text-neutral-500 font-mono">{{ block.language }}</span>
                      </div>
                      <pre class="p-5 overflow-x-auto text-sm text-green-400 font-mono leading-relaxed"><code>{{ block.code }}</code></pre>
                    </div>
                  </template>
                </div>

                <!-- Navigation -->
                <div class="mt-10 pt-6 border-t border-neutral-200/60 dark:border-neutral-800/60 flex items-center justify-between">
                  <button
                    v-if="sections.findIndex(s => s.id === key) > 0"
                    @click="activeSection = sections[sections.findIndex(s => s.id === key) - 1].id"
                  ...
                  <div v-else />
                  <button
                    v-if="sections.findIndex(s => s.id === key) < sections.length - 1"
                    @click="activeSection = sections[sections.findIndex(s => s.id === key) + 1].id"
