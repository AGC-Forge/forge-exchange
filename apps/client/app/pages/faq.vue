...
useSeoMeta({
  ...
  description: 'Find answers to the most common questions about Forge Exchange traffic exchange platform.',
})
...
const currentItems = computed(() => faqItems.value[activeCategory.value as keyof typeof faqItems.value] || [])
  ...
  <div>
    ...
    <section class="bg-white dark:bg-neutral-950 border-b border-neutral-200/50 dark:border-neutral-800/50">
      <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 lg:py-20 text-center">
        ...
        <p class="text-lg text-neutral-500 dark:text-neutral-400 max-w-2xl mx-auto mb-8">
          {{ t('faq.subtitle') }}
        </p>
        ...
        <div class="max-w-xl mx-auto">
          <div class="relative">
            <UIcon name="i-lucide-search" class="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-neutral-400" />
            <input
              v-model="searchQuery"
              type="text"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div class="lg:col-span-3">
            <div v-if="filteredItems.length === 0" class="text-center py-16">
              <UIcon name="i-lucide-search-x" class="h-12 w-12 text-neutral-300 dark:text-neutral-600 mx-auto mb-4" />
              <p class="text-neutral-500 dark:text-neutral-400">No results found for "{{ searchQuery }}"</p>
            </div>
            ...
            <div v-else class="space-y-3">
              ...
              >
                <UAccordion
                  :items="[{ label: item.q, content: item.a, defaultOpen: false }]"
                  variant="ghost"
                  class="w-full"
                >
                  <template #item="{ item }">
