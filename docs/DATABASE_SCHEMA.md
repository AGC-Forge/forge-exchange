...
> To add new month partition, run the following:
>
> ```sql
...
3. **Denormalized counters** in `campaigns` untuk performa dashboard
4. **Encrypted credentials** in `integrations` dan `proxy_pools.password`
5. **ip_hash** not raw IP in analytics - privacy-safe
6. **Composite indexes** di query path kritikal
